...
function getTwoFactorEmailSubject(type: 'LOGIN' | 'SETUP' | 'RESET', code: string): string {
  switch (type) {
    case 'LOGIN':
      return `Code de vérification ${code} pour votre connexion - Psychoz`;
    case 'SETUP':
      return `Code de vérification ${code} pour activer la 2FA - Psychoz`;
    case 'RESET':
      return `Code de vérification ${code} pour réinitialiser la 2FA - Psychoz`;
    default:
      return `Code de vérification ${code} - Psychoz`;
  }
}
...
export function generateTwoFactorEmailHTML(username: string, code: string, type: 'LOGIN' | 'SETUP' | 'RESET'): string {
  ...
  return `
    ...
    <head>
      ...
      <title>Code de vérification Psychoz</title>
      <style>
